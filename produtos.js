...
document.addEventListener('DOMContentLoaded', function() {
    ...
    paymentOptions.forEach(option => {
        option.addEventListener('change', function() {
            ...
            // Show/hide PIX QR Code based on selection
            if (this.value === 'PIX QR Code') {
                // Calculate total amount
                let total = 0;
                cart.forEach(item => {
                    total += item.price * item.quantity;
                });
                pixAmountElement.textContent = `R$ ${total.toFixed(2).replace('.', ',')}`;
    ...
    copyPixKeyBtn.addEventListener('click', function() {
        const pixKey = '31982517147';
        navigator.clipboard.writeText(pixKey).then(function() {
            showNotification('Chave PIX copiada com sucesso!', 'success');
        }).catch(function(err) {
            showNotification('Erro ao copiar a chave PIX. Por favor, copie manualmente.', 'warning');
        });
    });
    ...
    submitOrderBtn.addEventListener('click', function() {
        ...
        // If PIX QR Code is selected, add PIX information
        if (paymentMethod.value === 'PIX QR Code') {
            orderMessage += `%0A%0A*Informações para pagamento via PIX:*%0A`;
            orderMessage += `*Chave PIX:* 31982517147%0A`;
            orderMessage += `*Valor:* R$ ${total.toFixed(2).replace('.', ',')}%0A`;
            orderMessage += `*Destinatário:* Charcutaria Artesanal%0A`;
        }
